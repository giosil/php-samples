...
class nusoap_server extends nusoap_base {
  ...
  function invoke_method() {
    ...
    $verify = $this->verify_method($this->methodname, $this->methodparams);
    if (!$verify['result']) {
      // debug
      $this->debug('ERROR: request not verified against method signature');
      $this->result = 'fault: request failed validation against method signature';
      // return fault
      $this->fault('SOAP-ENV:Client', $verify['txt']);
      return;
    }
  ...
  function verify_method($operation, $request){
    ...
    //var_dump($this->wsdl->currentBinding);
    
    $return = array('result'=>false,'txt'=>"Operation '$this->methodname' not defined in service.");
    
    if (isset($this->wsdl) && is_object($this->wsdl)) {
      if($data = $this->wsdl->getOperationData($operation)){return $this->verify_params($data['input']['parts'],$request);
      }
    } elseif (isset($this->operations[$operation])) {
      return $this->verify_params($this->operations[$operation]['in'],$request);
    }
    
    return $return;
  }
  
  function verify_params($input, $request){ 

    if(count($input) != count($request)){
      foreach($input as $k =>$v){
        if(!array_key_exists($k, $request)){
          return array('result'=>false, 'txt'=>"Operation ('$this->methodname'). Number of request paramaters not match. Paramater ( $k ) not exist."); 
        } 
      }
    }else{
      foreach($request as $k =>$v){
        if(!array_key_exists($k, $input)){
          return array('result'=>false, 'txt'=>"Operation ('$this->methodname').The name of the parameter ( $k ) is wrong."); 
        } 
      }
    }
    return array('result'=>true,"txt"=>"");
  }
